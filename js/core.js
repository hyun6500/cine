...
/* ---------------- 상수 ---------------- */
const APP_VERSION = "2.18";   // 푸터에 표시 — 배포된 버전 확인용
...
function stampFooter(){
  const el = $("#appver");
  if (!el) return;
  const names = ["core","auth","poster","widgets","browse","dash","wall","diary","explore","search","wish","add","ticket","cal","story","main"];
...
/* ---------------- 비공개 항목 (v2.17) ----------------
   ★ 친구에게 화면을 보여줄 때 '누구와 봤는지'가 드러나지 않도록,
   ...
   ★★ CONFIG.PRIVATE_MODE(v2.18)가 켜져 있으면 서버가 아예 안 내려보냅니다.
     잠금 상태에서는 '동반/메모'가 브라우저에 도착조차 하지 않으므로
...
const S = {
  ...
  diary:   { shown:0, view:"list", ym:null, cat:null, day:null },
...
/* ---------------- 데이터 로드 ----------------
   ...
   ① 비공개 모드(CONFIG.PRIVATE_MODE) — Apps Script를 거쳐 읽는다.
      토큰이 없으므로 서버가 '동반/메모'를 지운 채 내려보낸다. 시트는 비공개여도 된다.
   ② 공개 모드 — 예전처럼 gviz CSV를 직접 읽는다. 빠르지만 시트가 공개여야 하고,
      동반 정보도 브라우저까지 내려온다(화면에서 가릴 뿐).
...
async function loadSheet(){
  ...
  try {
    ...
    boot();
    /* 이미 잠금이 풀린 상태(같은 탭에서 새로고침 등)면 동반 정보를 바로 받아 온다 */
...
/* ---------------- 비공개 칼럼 수신/폐기 (v2.18) ----------------
   잠금 해제 시 '동반/메모'만 따로 받아 기존 행에 덧입힌다 (전체 재수신 아님 — 응답이 작다).
...
/* ★ 쓰기 직전에 반드시 통과해야 하는 관문.
   동반 정보를 받아오기 전에 수정 창이 열리면 입력칸이 비어 있고,
   그대로 저장하면 시트의 동반 정보가 지워진다. 그 사고를 막는다. */
...
async function loadPrivate(){
  ...
  try {
    ...
    console.warn("[cine] 동반 정보를 받지 못했습니다", e);
    toast("동반 정보를 불러오지 못했습니다 — 잠시 후 다시 시도해 주세요", "warn");
  }
  _privLoading = false;
}
...
function renderStrip(){
  ...
  /* ★ '언젠가' 칸 — 관람 시점을 모르는 기록을 연도처럼 하나로 모아 본다.
     연도축에 올릴 수 없을 뿐 엄연히 본 작품이라, 아예 안 보이면 잊혀진다. */
