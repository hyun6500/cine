...
/* ---------------- 상수 ---------------- */
const APP_VERSION = "2.25";   // 푸터에 표시 — 배포된 버전 확인용
...
/* ---------------- 좋았던 영화 (v2.25) ----------------
   달력 칸의 금색 테두리와 '좋았던 영화' 목록이 같은 기준을 쓴다.
   ★ 별점은 회차마다 따로 남기 때문에(상세에서 매기면 가장 최근 회차에 저장),
     같은 작품의 모든 회차 중 가장 높은 별점을 그 작품의 별점으로 본다.
     안 그러면 두 번 본 영화의 첫 관람일 칸만 금색이 빠진다. */
const GOOD_RATE = 3.5;

function workRateMap(){
  const m = {};
  S.rows.forEach(r => {
    const v = parseRate(r.rate);
    if (v != null && (m[r.key] == null || v > m[r.key])) m[r.key] = v;
  });
  return m;
}
function isGoodIn(map, r){ const v = map[r.key]; return v != null && v >= GOOD_RATE; }

/* 한 달 기록에서 좋았던 영화만 작품 단위로 추린다 (같은 달 두 번 봤으면 먼저 본 날 기준) */
function goodOfMonth(rows, map){
  const seen = new Set(), out = [];
  rows.slice().sort((a,b) => a.date < b.date ? -1 : 1).forEach(r => {
    if (!isGoodIn(map, r) || seen.has(r.key)) return;
    seen.add(r.key);
    out.push({ r, rate: map[r.key] });
  });
  return out;
}
