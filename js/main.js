...
function init(){
  ...
  $("#btn-lock").onclick = () => {
    ...
    else ensureAuth("편집 잠금을 미리 해제해 둘 수 있습니다.");
  };
  updateLockUI();

  /* 검색 — 평소엔 돋보기, 누르면 펼쳐짐 */
  const sb = $("#searchbar");
  const openSearch = () => { sb.classList.add("open"); setTimeout(() => $("#q").focus(), 30); };
  const closeSearch = () => {
    sb.classList.remove("open");
    if (S.q){ S.q = ""; $("#q").value = ""; applyFilters(); }
  };
  $("#btn-search").onclick = () => sb.classList.contains("open") ? closeSearch() : openSearch();
  $("#q-close").onclick = closeSearch;
  $("#q").addEventListener("input", e => { S.q = e.target.value; applyFilters(); });
