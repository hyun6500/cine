...
function renderDash(){
  ...
  <div class="card c-6"><h3>넷플릭스 평가 분포 <span class="mono">MY RATINGS · 작품 기준</span></h3>
    ${nfTotal ? `<div class="hbar">
      ${["최고예요","좋아요","별로예요"].map(k=>`<div class="hb"><span class="nm">${NFLX[k].ico} ${k}</span><span class="tr"><span class="fl" style="width:${nf[k]/nfMax*100}%;background:${nfColors[k]}"></span></span><span class="vl">${nf[k]}</span></div>`).join("")}
