...
function renderDash(){
  ...
  /* ★ 막대는 CSS에 전혀 의존하지 않게 그린다 (구버전 CSS가 캐시돼도 렌더되도록)
     - span이 아니라 div (인라인이면 width %가 무시됨 — 이 앱의 원래 버그)
     - 높이는 부모 100%가 아니라 고정 px, 색은 CSS 변수가 아니라 헥스 */
  const TRACK = "display:block;width:100%;height:9px;background:#10141A;border-radius:99px;overflow:hidden";
  const BAR = (pct, color) =>
    `display:block;height:9px;border-radius:99px;min-width:3px;`
    + `width:${Math.max(Math.round(pct*10)/10, 1.5)}%;background:${color || "#F5B942"}`;
  const HB = (name, pct, val, color, attrs) =>
    `<div class="hb${attrs ? " clickable" : ""}"${attrs || ""}>`
    + `<span class="nm">${name}</span>`
    + `<div class="tr" style="${TRACK}"><div class="fl" style="${BAR(pct, color)}"></div></div>`
