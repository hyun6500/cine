...
function authLock(silent){
  ...
  /* 비공개 항목(동반/메모)을 즉시 다시 가린다 — 화면을 건네기 직전에 누르는 버튼이므로.
  ...
  if (!silent) toast("잠갔습니다 — 동반 정보가 가려지고, 수정·삭제·찜에 비밀번호가 다시 필요합니다");
}
...
function updateLockUI(){
  ...
  b.title = on
    ? `편집 가능 · 동반 정보 표시 중 — ${min}분 후 자동 잠금 (눌러서 바로 잠그기)`
    : "잠김 — 동반 정보가 가려져 있습니다. 편집하거나 보려면 비밀번호 입력";
}
setInterval(() => {
  if (AUTH.pw && !authAlive()){ authLock(true); toast("10분이 지나 자동으로 잠겼습니다 — 동반 정보도 다시 가려집니다"); }
  else updateLockUI();
}, 20000);

/* ---------- 비밀번호 입력 ---------- */
function openAuthModal(reason){
  $("#auth-msg").textContent = reason || "수정·삭제·찜하기와 동반 정보 보기에는 비밀번호가 필요합니다.";
...
async function trySubmitAuth(){
  ...
  try {
    ...
    if (d.ok){
      ...
      closeAuthModal(true);
      if (typeof refreshPrivacyViews === "function") refreshPrivacyViews();
      /* 동반 정보 수신은 ensureAuth가 이어서 처리한다 (중복 요청 방지) */
      toast("10분간 편집할 수 있고, 동반 정보가 표시됩니다");
    } else {
...
/* 쓰기 직전 호출 — true면 진행.
   ★ 비공개 모드에서는 통과 직후 동반 정보를 먼저 채운다.
     안 그러면 수정 창이 빈 동반칸으로 열리고, 저장하는 순간 시트 값이 지워진다. */
