...
function authLock(silent){
  ...
  /* 비공개 항목을 즉시 다시 가린다 — 화면을 건네기 직전에 누르는 버튼이므로.
     비공개 모드에서는 메모리에서도 지운다(다시 보려면 서버에서 새로 받아야 함). */
  if (typeof dropPrivate === "function") dropPrivate();
  if (typeof refreshPrivacyViews === "function") refreshPrivacyViews();
  if (!silent) toast("잠갔습니다 — 비공개 항목이 가려지고, 수정·삭제·찜에 비밀번호가 다시 필요합니다");
}
...
function updateLockUI(){
  ...
  b.title = on
    ? `편집 가능 — ${min}분 후 자동 잠금 (눌러서 바로 잠그기)`
    : "잠김 — 편집하려면 비밀번호 입력";
}
setInterval(() => {
...
function openAuthModal(reason){
  ...
  /* 뒤로가기로 닫으면 '취소'로 처리한다 (대기 중인 약속을 false로 풀어줘야 멈추지 않는다) */
  navOpen("auth", () => closeAuthModalDom(false));
  setTimeout(() => $("#auth-pw").focus(), 50);
}

function closeAuthModal(ok){
  if (navClose("auth")){
    /* popstate가 DOM을 닫되, 성공(ok=true)이면 그 결과를 잃지 않도록 미리 넘긴다 */
    if (ok){ const r = AUTH._resolve; AUTH._resolve = null; if (r) r(true); }
    return;
  }
  closeAuthModalDom(ok);
}

function closeAuthModalDom(ok){
...
async function trySubmitAuth(){
  ...
  try {
    ...
    if (d.ok){
      ...
      closeAuthModal(true);
      if (typeof refreshPrivacyViews === "function") refreshPrivacyViews();
      /* 비공개 칼럼 수신은 ensureAuth가 이어서 처리한다 (중복 요청 방지) */
...
/* 쓰기 직전 호출 — true면 진행.
   ★ 비공개 모드에서는 통과 직후 비공개 칼럼을 먼저 채운다.
     안 그러면 수정 창이 빈 칸으로 열리고, 저장하는 순간 시트 값이 지워진다. */
async function ensureAuth(reason){
  if (authAlive()){
    authTouch();
    if (typeof ensurePrivateLoaded === "function") await ensurePrivateLoaded();
    return true;
  }
  openAuthModal(reason);
  const ok = await new Promise(res => { AUTH._resolve = res; });
  if (ok && typeof ensurePrivateLoaded === "function") await ensurePrivateLoaded();
  return ok;
}
