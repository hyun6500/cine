...
function dayShot(rs, goodMap){
  ...
  use.forEach(r => {
    ...
    /* 좋았던 영화(별점 3.5 이상)는 조각 안쪽에 금색 테두리 (v2.25) */
    t.className = "sh" + (goodMap && isGoodIn(goodMap, r) ? " good" : "");
...
function renderDiaryCal(list){
  ...
  for (let i = 0; i < cells; i++){
    ...
    if (rs.length){
      ...
      cell.insertBefore(dayShot(rs, goodMap), cell.firstChild);
      cell.onclick = () => {
  ...
  /* 좋았던 영화 (v2.25) — 이 달에 본 것 중 별점 3.5 이상, 제목과 감독 */
  const goods = goodOfMonth(list.filter(r => r.date.slice(0,7) === D.ym), goodMap);
  if (goods.length){
    const gb = $("#cal-good");
    gb.className = "cal-goodbox";
    gb.innerHTML = `<h3>좋았던 영화 <span>${goods.length}편 / 별점 ${GOOD_RATE} 이상</span></h3>`
      + goods.map(({r, rate}, i) => `<button class="gd" data-i="${i}">
          <span class="gd-d">${esc(r.date.slice(5).replace("-","."))}</span>
          <span class="gd-t">${esc(r.title)}${seasonTag(r)}</span>
          <span class="gd-m">${esc(r.dir ? r.dir.split(",")[0].trim() : "")}</span>
          <span class="gd-r">${Number(rate).toFixed(1)}</span>
        </button>`).join("");
    gb.querySelectorAll(".gd").forEach(b => b.onclick = () => openModal(goods[+b.dataset.i].r));
  }
