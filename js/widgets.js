...
function openModal(r){
    ...
    <div class="mo-info">
      ...
      <div class="mo-log">
        ...
        ${all.map(v => `<div class="viewing">
          ...
          ${v.memo && canSeePrivate() ? `<span class="k">MEMO${PRIVATE_MARK}</span><span class="v priv">${esc(v.memo)}</span>` : ""}
          ${v.rate ? `<span class="k">RATE</span><span class="v">★ ${esc(rateStr(parseRate(v.rate)) || v.rate)}</span>` : ""}
          <span class="k"></span><span class="v vbtns"><button class="vedit" data-no="${esc(v.no)}">수정</button><button class="vedit vdel" data-no="${esc(v.no)}">삭제</button><span class="calslot" data-no="${esc(v.no)}"></span></span>
          </div>
        </div>`).join("")}
  ...
  $("#modal-body").querySelectorAll(".vedit:not(.vdel)").forEach(b => b.onclick = () => {
    ...
    /* ★ 수정 창에는 비공개 메모 입력칸이 있으므로, 창을 여는 것 자체에 잠금을 건다.
       (친구가 화면을 만지다 '수정'을 눌러 값을 보게 되는 구멍) */
    if (!v) return;
    /* ensureAuth가 비공개 칼럼 수신까지 마친 뒤에 창을 연다 (빈 값 저장 사고 방지) */
    ensureAuth("수정 창에는 비공개 메모가 함께 표시됩니다. 비밀번호를 입력해 주세요.")
      .then(ok => { if (ok) openEdit(v); });
  });
  ...
  /* 회차별 캘린더 등록 (v2.15) — 날짜가 있는 기록만 */
  $("#modal-body").querySelectorAll(".calslot").forEach(sl => {
    const v = all.find(x => String(x.no) === sl.dataset.no);
    if (v && v.date && !v.undated) sl.appendChild(calBtn(v));
  });
...
function openPicker(defaultQ, typeHint, onPick){
  ...
  const choose = async (x, mt) => {
    const m = await workMeta(mt, x.id);
    closePicker();
    onPick({
      tmdb: String(x.id), ntype: mt,
      title: m.title || x.title || x.name,
      year: m.year || (x.release_date || x.first_air_date || "").slice(0,4),
      dir: m.dirs, genre: m.genres, nation: m.countries,
      poster: m.poster || x.poster_path || "",
    });
  };
...
function closePicker(){
  if (navClose("picker")) return;
  closePickerDom();
}

function closePickerDom(){
...
/* 잠금/해제 순간에 화면을 다시 그려 비공개 항목을 즉시 감추거나 드러낸다.
   (모달을 열어둔 채 자물쇠를 눌렀는데 값이 그대로 남아 있으면 곤란하다) */
function refreshPrivacyViews(){
  if (modalRow && $("#modal-bg").classList.contains("show")) openModal(modalRow);
  if (S.tab === "explore" && typeof renderExplore === "function"){
    try { renderExplore(); } catch(e){}
  }
}

function closeModal(){
  if (navClose("modal")) return;      // 실제 닫기는 popstate가 한다
  closeModalDom();
}
