...
function openModal(r){
    ...
    <div class="mo-info">
      ...
      <div class="mo-log">
        ...
        ${all.map(v => `<div class="viewing">
          ...
          ${v.memo && canSeePrivate() ? `<span class="k">WITH${PRIVATE_MARK}</span><span class="v priv">${esc(v.memo)}</span>` : ""}
  ...
  $("#modal-body").querySelectorAll(".vedit:not(.vdel)").forEach(b => b.onclick = () => {
    ...
    /* ★ 수정 창에는 동반/메모 입력칸이 있으므로, 창을 여는 것 자체에 잠금을 건다.
       (친구가 화면을 만지다 '수정'을 눌러 값을 보게 되는 구멍) */
    if (!v) return;
    /* ensureAuth가 동반 정보 수신까지 마친 뒤에 창을 연다 (빈 값 저장 사고 방지) */
    ensureAuth("수정 창에는 동반/메모가 함께 표시됩니다. 비밀번호를 입력해 주세요.")
...
/* 잠금/해제 순간에 화면을 다시 그려 비공개 항목을 즉시 감추거나 드러낸다.
   (모달을 열어둔 채 자물쇠를 눌렀는데 동반 정보가 그대로 남아 있으면 곤란하다) */
