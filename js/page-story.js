...
async function drawCalCell(ctx, x, y, w, h, dnum, list, scale){
  ...
  /* 대표 한 편 — 별점이 있으면 가장 높은 것, 없으면 첫 번째 */
  const lead = has
    ? list.slice().sort((a,b) => (parseRate(b.rate) ?? -1) - (parseRate(a.rate) ?? -1))[0]
    : null;

  ctx.save();
  roundRect(ctx, x, y, w, h, rad); ctx.clip();
  if (has){
    const p = await posterFor(lead);
    ...
    if (im) drawCover(ctx, im, x, y, w, h);
      ...
      ctx.font = `600 ${Math.round(15*scale)}px ${ST_FONT}`;
      ctx.fillText(ellipsis(ctx, lead.title, w - 12), x + w/2, y + h/2);
    }
