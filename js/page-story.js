...
function closeStory(){
  if (navClose("story")) return;
  closeStoryDom();
}

function closeStoryDom(){
...
async function drawCalStory(ctx, rows, top){
  ...
  /* 앱 달력과 같은 순서로 쪼갠다 (직접 정한 순서 > 상영시각 > 기록 순) */
  if (typeof dayOrder === "function") Object.keys(byDay).forEach(k => { byDay[k] = dayOrder(byDay[k]); });

  const goodMap = workRateMap();
  const goods = goodOfMonth(rows, goodMap);
  ...
  /* ★ v2.25: 아래 빈 공간에 '좋았던 영화' 목록을 둔다.
     목록이 들어갈 높이를 먼저 빼고 남은 만큼 격자에 준다.
     달력이 주인공이므로 격자는 80% 밑으로 줄이지 않고, 대신 목록 줄 수를 줄인다(나머지는 '외 N편'). */
  const LIST_MAX = 8, ROW_H = 54, HEAD_H = 84, GAP_LIST = 40, MIN_SCALE = 0.8;
  let shown = 0, listH = 0, scale = 1;
  const fit = (n) => {
    const lh = n ? HEAD_H + n * ROW_H + (goods.length > n ? 44 : 0) : 0;
    const avail = bottom - top - wdH - (n ? GAP_LIST : 0) - lh;
    return { lh, sc: Math.min(1, avail / needH) };
  };
  for (let n = Math.min(goods.length, LIST_MAX); n >= 0; n--){
    const f = fit(n);
    if (f.sc >= MIN_SCALE || n === 0){ shown = n; listH = f.lh; scale = f.sc; break; }
  }

  const w = cellW * scale, h = cellH * scale;
  const gridW = w * 7 + gap * 6;
  const gridH = h * weeks + gap * (weeks - 1);
  const x0 = (ST_W - gridW) / 2;

  /* 격자 + 목록 묶음을 세로 가운데로 */
  const blockH = wdH + gridH + (shown ? GAP_LIST + listH : 0);
  const yTop = top + Math.max(0, (bottom - top - blockH) / 2);
  ...
  for (let i = 0; i < weeks * 7; i++){
    ...
    await drawCalCell(ctx, x, y, w, h, dnum, list, scale, goodMap);
  }

  if (shown) drawGoodList(ctx, goods, shown, x0, y0 + gridH + GAP_LIST, gridW);
}

/* 좋았던 영화 목록 — 날짜 / 제목 / 감독 / 별점 한 줄씩 */
const GOLD = "#E8B04A";
function drawGoodList(ctx, goods, shown, x, y, width){
  const right = x + width;

  /* 제목줄 + 금색 선 */
  ctx.textAlign = "left"; ctx.textBaseline = "alphabetic";
  ctx.fillStyle = GOLD;
  ctx.font = `700 34px ${ST_DISP}`;
  ctx.fillText("좋았던 영화", x, y + 36);
  const hw = ctx.measureText("좋았던 영화").width;
  ctx.fillStyle = "#6C7686";
  ctx.font = `400 20px 'IBM Plex Mono', monospace`;
  ctx.fillText(`${goods.length}편 / 별점 ${GOOD_RATE} 이상`, x + hw + 16, y + 36);
  ctx.strokeStyle = "rgba(232,176,74,.45)"; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(x, y + 58); ctx.lineTo(right, y + 58); ctx.stroke();

  let yy = y + 84 + 18;
  for (let i = 0; i < shown; i++){
    const { r, rate } = goods[i];
    /* 별점 (오른쪽 끝) */
    ctx.textAlign = "right";
    ctx.fillStyle = GOLD;
    ctx.font = `600 24px 'IBM Plex Mono', monospace`;
    ctx.fillText(Number(rate).toFixed(1), right, yy);   // 3.5 / 4.0 자리 맞춤
    const rw = ctx.measureText("0.0").width + 24;

    /* 날짜 (왼쪽) */
    ctx.textAlign = "left";
    ctx.fillStyle = "#6C7686";
    ctx.font = `400 22px 'IBM Plex Mono', monospace`;
    ctx.fillText(r.date.slice(5).replace("-", "."), x, yy);
    const dx = x + 92;

    /* 감독 (별점 왼쪽에 붙여 오른쪽 정렬) */
    const dir = r.dir ? r.dir.split(",")[0].trim() : "";
    ctx.font = `400 24px ${ST_FONT}`;
    const dirText = dir ? ellipsis(ctx, dir, width * 0.30) : "";
    const dw = dirText ? ctx.measureText(dirText).width : 0;
    ctx.textAlign = "right";
    ctx.fillStyle = "#8B93A1";
    if (dirText) ctx.fillText(dirText, right - rw, yy);

    /* 제목 (남은 폭) */
    ctx.textAlign = "left";
    ctx.fillStyle = "#E8E6E1";
    ctx.font = `600 28px ${ST_FONT}`;
    ctx.fillText(ellipsis(ctx, r.title, right - rw - dw - 24 - dx), dx, yy);

    yy += 54;
  }
  if (goods.length > shown){
    ctx.textAlign = "left";
    ctx.fillStyle = "#6C7686";
    ctx.font = `400 22px ${ST_FONT}`;
    ctx.fillText(`외 ${goods.length - shown}편`, x + 92, yy - 8);
  }
}
...
async function drawCalCell(ctx, x, y, w, h, dnum, list, scale, goodMap){
  const T = S.story;
  const rad = 10 * scale;
  ...
  if (has){
    ...
    for (let i = 0; i < use.length; i++){
      ...
      /* 좋았던 영화 조각은 기억해 뒀다가 맨 마지막에 금색 테두리를 긋는다
         (날짜 그늘, 별점 그늘이 나중에 덮여서 테두리가 흐려지는 것을 막기 위해) */
      if (goodMap && isGoodIn(goodMap, use[i])) goldRects.push([rx, ry, rw, rh]);
      ctx.restore();
    }
  ...
  /* 좋았던 영화 금색 테두리 — 마지막에 그어야 그늘에 덮이지 않는다 */
  if (goldRects.length){
    const lw = Math.max(3, 4 * scale);
    ctx.strokeStyle = GOLD; ctx.lineWidth = lw;
    if (use.length === 1){
      roundRect(ctx, x + lw/2, y + lw/2, w - lw, h - lw, Math.max(2, rad - lw/2)); ctx.stroke();
    } else {
      ctx.save();
      roundRect(ctx, x, y, w, h, rad); ctx.clip();
      goldRects.forEach(([rx, ry, rw, rh]) => ctx.strokeRect(rx + lw/2, ry + lw/2, rw - lw, rh - lw));
      ctx.restore();
    }
  }
}
