...
async function drawStoryInner(){
  ...
  ctx.fillText("주현  시네마테크", ST_W/2, 168);
  ctx.fillStyle = "#E8E6E1";
  ctx.font = `700 66px ${ST_DISP}`;
  ctx.fillText(ellipsis(ctx, T.title || "요즘 본 것들", ST_W - 140), ST_W/2, 262);
  ...
  ctx.fillText(sub, ST_W/2, 314);

  ctx.strokeStyle = "rgba(245,185,66,.35)"; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(ST_W/2 - 48, 348); ctx.lineTo(ST_W/2 + 48, 348); ctx.stroke();

  if (!picked.length){ paintPreviewSize(); return; }

  if (T.layout === 1) await drawSingle(ctx, picked[0]);
  else await drawGrid(ctx, picked, L);

  /* 꼬리말 */
  ctx.textAlign = "center";
  ctx.fillStyle = "#5A6270";
  ctx.font = `400 22px 'IBM Plex Mono', monospace`;
  ctx.fillText("MY CINEMATHEQUE · " + new Date().toISOString().slice(0,10).replace(/-/g,"."), ST_W/2, ST_H - 74);
  paintPreviewSize();
}

/* 격자 (4·9·12·16) */
async function drawGrid(ctx, picked, L){
  const T = S.story;
  const top = 400, bottom = ST_H - 130;
  const padX = 64, gap = L.cols >= 4 ? 20 : 26;
  ...
  for (let i = 0; i < picked.length; i++){
    ...
    await drawTile(ctx, r, cx, cy, w, posterH, scale, L);
  }
}
...
async function drawSingle(ctx, r){
  ...
  const maxW = ST_W - 150;
  if (ctx.measureText(r.title).width > maxW){
    const words = r.title.split(" ");
    let l1 = "", l2 = "";
    words.forEach(w => {
      if (ctx.measureText(l1 + " " + w).width < maxW && !l2) l1 = (l1 ? l1 + " " : "") + w;
      else l2 = (l2 ? l2 + " " : "") + w;
    });
    ctx.fillText(l1, ST_W/2, y);
    if (l2){ y += 64; ctx.fillText(ellipsis(ctx, l2, maxW), ST_W/2, y); }
