...
function openSearch(preset){
  ...
  navOpen("search", closeSearchDom);      // 뒤로가기 처리는 nav.js가 맡는다
  const inp = $("#q");
...
function closeSearch(){
  if (navClose("search")) return;
  closeSearchDom();
}

function closeSearchDom(){
  const ov = $("#search-overlay");
...
function runSearch(){
  ...
  if (wg) wished.forEach(w => {
    ...
    c.onkeydown = e => { if (e.key === "Enter") c.onclick(); };
    if (!wishWatched(w)) c.appendChild(logBtn(w));
    wg.appendChild(c);
  });
...
async function searchTmdb(q){
  ...
  try {
    ...
    list.forEach(x => {
      ...
      if (!row) c.appendChild(logBtn(work));      // 아직 기록에 없는 작품만 ＋ 노출
      c.onclick = () => {
...
function openTmdbDetail(work){
  ...
  /* 감독·장르는 상세 조회로 채워 기록에 함께 저장되게 (TV는 에피소드 크레딧까지 훑는다) */
  if (work.tmdb && tmdbReady()){
    (async () => {
      const m = await workMeta(work.ntype, work.tmdb);
      work.genre = m.genres; work.dir = m.dirs; work.nation = m.countries;
      if (m.title) work.title = m.title;
      const el = $("#td-meta");
...
/* ---------------- ＋ 본 작품으로 기록 ----------------
   ★ 찜(♡)은 "앞으로 볼 것", ＋는 "이미 본 것". 검색 결과에서 두 갈래가 나뉜다.
     대부분 관람 시점을 기억하지 못하므로 기본은 '날짜 미상' — 연도축의 '언젠가' 칸에 모인다. */
function logBtn(work){
  const b = document.createElement("button");
  b.className = "lbtn";
  b.innerHTML = "＋";
  b.title = "봤어요 — 기록 추가 (날짜를 몰라도 됩니다)";
  b.onclick = async e => {
    e.stopPropagation();
    b.disabled = true; b.innerHTML = "…";
    const w = { ...work };
    /* 감독·장르·국가는 목록 검색 결과에 없으므로 상세를 한 번 더 캔다 */
    if (w.tmdb && !w.dir && tmdbReady()){
      try {
        const m = await workMeta(w.ntype || "movie", w.tmdb);
        w.dir = m.dirs; w.genre = m.genres; w.nation = m.countries;
        if (m.title) w.title = m.title;
        if (m.year) w.year = w.year || m.year;
        if (m.poster) w.poster = w.poster || m.poster;
      } catch(err){}
    }
