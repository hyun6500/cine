...
function openEdit(r){
  ...
  $("#edit-body").innerHTML = `
    ...
    <div class="ed-sec"><div class="ed-h">관람 정보 <span class="sub">이 회차만</span></div>
      ...
      <div class="frow">
        <div><label class="fl">메모</label><input type="text" id="e-memo" class="fin" value="${esc(r.memo)}"></div>
...
function renderEditWork(){
  ...
  $("#ed-pick").onclick = () => openPicker(editRow.title, editRow.ntype, work => {
    editWork = work;
    /* ★ TMDB와 연결되면 그 작품의 공식 제목을 그대로 쓴다 (확인 절차 없음).
       표기가 한 곳으로 통일돼야 검색·중복점검·재관람 묶임이 일관되게 동작한다. */
    if (work.title) $("#e-title").value = work.title;
    ...
    box.style.display = "";
    box.innerHTML = `제목을 TMDB 표기 <b>${esc(work.title || "")}</b> 로 맞췄습니다.`;
  });
}

function closeEdit(){
  if (navClose("edit")) return;
  closeEditDom();
}

function closeEditDom(){
...
function openQuickLog(work){
  ...
  $("#edit-body").innerHTML = `
    ...
    <div class="ed-sec"><div class="ed-h">언제 봤나요? <span class="sub">기억나지 않으면 그대로 두세요</span></div>
      <label class="ed-chk"><input type="checkbox" id="ql-nodate" checked> 관람 시점이 기억나지 않음
        <span class="sub">(연도 필터의 ‘${UNDATED_LB}’ 칸에 모입니다)</span></label>
      ...
      <div class="frow" style="margin-top:8px">
        <div><label class="fl">플랫폼/상영관 <span class="sub">선택 · 날짜를 몰라도 기억나면</span></label>
          <input type="text" id="ql-plat" class="fin" placeholder="넷플릭스, CGV 등"></div>
      </div>
    </div>
    ...
    <p class="sub-p tiny">날짜 미상으로 저장하면 다이어리·월별 통계에서는 빠지지만,
      필름스트립·연도 셀렉트의 <b>‘${UNDATED_LB}’</b> 칸에 한데 모여 언제든 다시 꺼내 볼 수 있습니다.
      포스터 월·별점·작품 목록에도 그대로 들어갑니다. 나중에 날짜가 생각나면 상세에서 수정하세요.</p>`;

  const stars = starWidget(work.rate || null, null);
...
async function submitQuickLog(work, stars){
  ...
  try {
    const res = await gsPost({ action:"add", rows:[row],
      reqId: "ql-" + Date.now() + "-" + Math.random().toString(36).slice(2,8), dedupe: true });
...
async function pickTmdb(x){
  ...
  $("#f-results").innerHTML = "";
  $("#f-search").value = "";
  const m = await workMeta(x.media_type, x.id);
  addSel.genreT = m.genres; addSel.dirT = m.dirs;
  $("#f-picked").innerHTML = `
...
async function submitSingle(){
  ...
  try {
    ...
    const res = await gsPost({ action:"add", rows:[row], reqId, dedupe: true });
    if (res.skipped && !res.added){ toast("이미 같은 기록이 시트에 있습니다", "warn"); }
    else {
      toast(`‘${title}’ 저장 완료 — 시트에 추가됐습니다`);
      addLocal([row], res.firstNo);
      if ($("#f-cal")?.checked)
        await calAdd([calEvent({ ...row, dir: row.dirT, genre: row.genreT })]);
      clearSingle();
    }
  } catch(e){
    toast("저장 실패: " + e.message, "warn"); console.error(e);
  ...
  for (const c of csvCands){
    ...
    try {
      ...
      if (exact.length === 1){
        ...
        const mm = await workMeta(ty, m.id);
        c.genreT = mm.genres; c.dirT = mm.dirs; c.nation = mm.countries;
        ok++;
...
async function csvSend(){
  ...
  try {
    const reqId = "csv-" + Date.now() + "-" + Math.random().toString(36).slice(2,8);
    const r = await gsPost({ action:"bulk", rows, reqId, dedupe: true });
    toast(`${r.added}건이 시트에 추가됐습니다` + (r.skipped ? ` · 중복 ${r.skipped}건 제외` : ""));
    addLocal(r.savedIdx ? r.savedIdx.map(i => rows[i]).filter(Boolean) : rows, r.firstNo);
